    ...
    Select Case MenuList.ListIndex
        Case 0
           ...
           CRlabel.Visible = False
            TMImage.Visible = False
            PrImage.Visible = False
        ...
        weiqianList.Visible = False
           ...
           CRlabel.Visible = False
            ...
            PrImage.Visible = False
        Case 5
            CRlabel.Caption = App.ProductName & Chr(10) & "Version " & App.Major & "." & App.Minor & App.Revision & Chr(10) & App.LegalCopyright & "感谢施振展同志提出的建设性意见"
            CRlabel.Visible = True
            TMImage.Visible = True
            PrImage.Visible = True
            Me.Width = 5130
            Me.Height = 4275
        weiqianList.Visible = False
            mingdanname.Visible = False
...
Private Sub outputqr_Click()
    ...
    If TimeValue(nowtimetext.Text) < DateTime.Time Then
    ccc = MsgBox("当前时间已迟于划线时间了，如果进行重新划线，会造成前面已签到但迟到的人员不在划线范围内。是否继续进行重新划线？", vbYesNo, "班级签到系统 - 应用设置")
    If ccc = vbYes Then
